  ...
  return (
    <div data-testid="worksheet-view" className="h-full flex flex-col min-h-0">
      <div className="flex-shrink-0 flex flex-wrap items-center gap-3 px-3 py-2 border-b border-slate-800 text-xs text-slate-300">
        ...
        {!showAudit && data && !isError && <span data-testid="ws-count" className="text-slate-500">{shown.length} rows</span>}
        <button type="button" data-testid="ws-audit-toggle" aria-pressed={showAudit} onClick={() => setShowAudit((v) => !v)}
          className={`px-2 py-1 rounded text-slate-100 ${showAudit ? 'bg-blue-600 hover:bg-blue-500' : 'bg-slate-700 hover:bg-slate-600'}`}>
          Audit log
        </button>
        {!showAudit && (
          <>
      ...
      {showAudit ? (
        <div className="flex-1 min-h-0"><WorksheetAuditView projectId={projectId} /></div>
      ) : (
