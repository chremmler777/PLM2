...
**Flaky test note (2026-09-18):** `tests/test_implementation_tracking.py::test_a_newer_clean_report_clears_the_open_risk` failed in two full-suite runs on the merged main and passed alone (24/24) and in the identical-tree branch run (890/890). Pre-existing, order/timing dependent, untouched by this work. Same family as the `test_weight_estimate` flake seen once during the build.
