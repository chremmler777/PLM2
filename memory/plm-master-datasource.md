...
- **BOM lives in `KWA.MATERIAL`, NOT STULIKO/STULIPO (those are EMPTY in this instance).** MATERIAL (8273 rows): `STLAPLNR`=parent article, `ARTNR`=child, `MENGE`+`MENGEEINH`=qty+unit, `VERSCHLEISSTEIL`/`ERSATZTEIL`=wear/spare. Component names come from the child's own ARTIKEL row (MATERIAL.BENENNUNG is usually null). ARTIKEL.STUELINR/APLNR just self-reference the ARTNR.
- **Make/buy signal = `ARTIKEL.EIGENKZ` (own-manufacture flag), NOT TEILEART.** EIGENKZ=1 → in-house (internal_mfg / sub_assembly); 0 → purchased. Critical: 50-series clips/fasteners are TEILEART='T' but EIGENKZ=0 → they are PURCHASED. TEILEART only distinguishes W=tool vs the rest. `DISPOSTUFE` = BOM depth level.
- **SP-A + SP-B DONE Jul 12 2026** (branch `feature/wincarat-import-g65-backfill`): live Postgres wiped clean then loaded from WinCarat — 29 active projects, 1934 parts +110 BOM-child parts, 2044 WC-IMP baseline revisions, 7477 BOM items with quantities, 1307 tool→article produces relations. Scripts: `backend/scripts/reset_transactional.py`, `wincarat_extract.py` (runs in twos container), `import_wincarat.py` (runs in plm2 container with PYTHONPATH=/app). All idempotent. NOTE: `app/main.py` startup re-seeds an empty "Test Project" + test org/main-plant + login users (admin@/test@example.com) on every boot — harmless empty project, do not delete the seed (provides logins).
...
**Decision Sep 17 2026 — WinCarat is a one-time bulk source, PLM is master from here.** New projects (1994, 2077 backpanel) are created in PLM first and later become the base for WinCarat; WinCarat itself is never written to (read-only, always). Consequence for revision naming (customer data index, spec `docs/superpowers/specs/2026-09-17-customer-data-index-design.md`): every `WC-IMP` baseline revision is released series data → migration 072 renames it to official `1` with `source='import'` and moves the part to `lifecycle_phase='series'`. Later ECRs on those parts are `1.1`, `1.2`. Applied to dev Postgres Sep 17 2026: 2075 baselines, 43 RFQ1→E1, 1 ECR→1.1.
