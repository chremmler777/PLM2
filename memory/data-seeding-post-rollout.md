...
- **MERGED to main Jul 20 2026** (merge commit 57185bce, pushed).
- **VW426 DONE Jul 20 2026** on branch `feature/vw426-import-backfill` (commit 2f9e3a9e, NOT yet merged). VW426 Atlas = WinCarat project **1864** (NULL PROJEKTSTATUS → skipped by the SERIES/PRE-SERI import filter; nothing was deleted). Added `--projects` override to `wincarat_extract.py`, imported 1864 (31 parts, 28 BOM, 12 produces rels). Sheets use a **different template** ("OPM Parts Resume", data from row 18, internal part in cell T8) → new `backfill_vw426_changes.py`; 72 closed pre-rollout changes, all part-linked via T8 (1:1, no fuzzy mapping). Repeated in-sheet row No.s get a stable `-N` occurrence suffix. `g65_backfeed_tasklist.py` now takes `--project-code`; per-program tasklists: G65=41, VW426=72. Raw customer xlsx left untracked (G65 precedent).
