...
const node = (over: Partial<BomNode>): BomNode => ({
  ...
  revision_id: 1, revision_name: 'E1', revision_phase: 'review', customer_index: null, cycle: false, lines: [], ...over,
})
...
const tree: BomNode = node({
  lines: [
    { id: 10, item_number: '10', name: 'SUB', quantity: 2, unit: 'pcs', total_quantity: 2, child_part_id: 2, catalog_part_id: null,
      child: node({ part_id: 2, part_number: 'SUB', name: 'Sub', revision_name: '1', revision_phase: 'official', customer_index: 'C',
...
describe('BomTree', () => {
  ...
  it('renders nested lines with multiplied totals and badges', () => {
    ...
    expect(screen.getByTestId('bom-rev-10').textContent).toBe('1 · C')
